...
def train(args, train_dataset, train_dataloader, model, tokenizer, labels, pad_token_label_id, data_dir, output_dir,
          t_logits, out_new_labels):
    """ Train the model """
    ...
    logger.info("  Total train batch size (w. parallel, distributed & accumulation) = %d",
                # 考虑到并行训练、分布式训练和累积梯度更新后的总训练批处理大小。
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                # 该函数会计算所有梯度的范数，并将其裁剪为指定的最大范数 args.max_grad_norm。
                    ...
                    torch.nn.utils.clip_grad_norm_(model.parameters(),
                                                   args.max_grad_norm)  # 则对model.parameters()中的梯度进行裁剪。
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        _, results, _, _ = evaluate(args, model, tokenizer, labels, pad_token_label_id, mode="dev",
                                                 data_dir=data_dir)
...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, mode, data_dir, prefix=""):
    ...
    support_encodings, support_labels = get_support_encodings_and_labels_total(
            args, model, support_dataloader, train_dataloader, pad_token_label_id)
    ...
    macro_results, micro_results, results = compute_metrics(metric)
    ...
    return macro_results, micro_results, preds_list, results
...
def get_rehearsal_prototype(args, model, tokenizer, labels, pad_token_label_id, mode, data_dir):
    ...
    support_encodings, support_labels = get_support_features_and_labels(
        args, model, support_dataloader, pad_token_label_id)
...
def load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode, data_dir):
    ...
    cached_features_file = os.path.join(data_dir, "cached_{}_{}_{}".format(mode, list(filter(None,args.model_name_or_path.split("/"))).pop(),
...
def get_support_encodings_and_labels_total(args, model, support_loader, train_loader, pad_token_label_id):
    """
...
def get_support_features_and_labels(args, model, support_loader, pad_token_label_id):
    """
    Get token encodings and labels for all tokens in the support set
    """
    support_encodings, support_labels = [], []
...
def train_and_eval(args, labels, num_labels, pad_token_label_id, model_name_or_path, output_dir, data_dir, step_id):
    ...
    # Training
    # 加载上一轮模型的参数，分词器，模型
    config = BertConfig.from_pretrained(args.config_name if args.config_name else model_name_or_path, num_labels = num_labels)
    tokenizer = BertTokenizer.from_pretrained(model_name_or_path)
    # from_tf=bool(".ckpt" in model_name_or_path) 用于指定是否从 TensorFlow 检查点加载模型
    model = MySftBertModel.from_pretrained(model_name_or_path, from_tf=bool(".ckpt" in model_name_or_path), config=config)
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model = MySftBertModel.from_pretrained(checkpoint, mode="dev")
            model.to(args.device)
            train_dataloader = None
            _, result, _, _ = evaluate(args, model, tokenizer, labels, pad_token_label_id, mode="dev",
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        tokenizer = BertTokenizer.from_pretrained(output_dir)
        model = MySftBertModel.from_pretrained(output_dir, mode="test")
        model.to(args.device)

        # data_dir = args.data_dir
        macro_results, micro_results, predictions, results = evaluate(args, model, tokenizer, labels,
                                                             pad_token_label_id,
        ...
        with open(output_test_predictions_file, "w", encoding="utf-8") as writer:
            ...
            for item in data:
                ...
                if predictions[sample_id]:
                    writer.write(f"sample_id:{sample_id}\n")
                    for word in text:
                        output_line = word + " " + predictions[sample_id].pop(0)[2:] + "\n"
                        writer.write(output_line)
                    
                else:
                    logger.warning("No predictions available for sample_id %d", sample_id)      
    return results
