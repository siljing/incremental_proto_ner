...
# from util.gather import selectgather_rh_ce_anchor_index
import numpy as np
...
class SupConLoss(Module):
    ...
    def forward(self, features, labels=None, mask=None, entity_topk=None,
        ...
        if len(features.shape) > 3:  # n_views大于 2，则将其展平成二维特征。
        ...
        if labels is not None and mask is not None:
            ...
            labels = labels.contiguous().view(-1, 1)  # 使张量在内存中连续存储，将 labels 重塑为一个二维张量，其中每行包含一个标签。
        ...
        anchor_dot_contrast = torch.div(
            ...
            self.temperature)   # [batch_size * n_views, batch_size * n_views]
        # for numerical stability
        logits_max, _ = torch.max(anchor_dot_contrast, dim=1, keepdim=True)
        logits = anchor_dot_contrast - logits_max.detach()

        # tile mask mask原来为[batch_size, batch_size],重复使其与 logits 具有相同的维度[batch_size * n_views, batch_size * n_views]
        mask = mask.repeat(anchor_count, contrast_count)
        # mask-out self-contrast cases掩蔽掉与自身的对比
        ...
        # select anchor 将标签为 ignore_index 的样本所对应的行的 mask 和 logits_mask 设置为全 0
        ...
        if entity_topk is not None:
            ...
            idx_o = torch.where(labels.view(-1) == 0)[0] # 标签张量中值为 0 的元素的索引。
            # 将logits_mask中idx_o列设为0
            ...
            logits_mask_o = torch.scatter(logits_mask_o, 1,# 将entity_topk索引所在行设为1.
                                          entity_topk.long(), 1)
...
class BceLoss(Module):
    ...
    def forward(self, output, labels, num_labels, old_target=None, cal_O=False):
        ...
        if old_target is None:
            ...
            old_target = old_target.to(device)
